...
$('#edit-slide-image-upload').click(function(){var myTop=$(this).offset();tb_show(_total_slider_L10n.uploadSlideBgImage,'media-upload.php?total-slider-uploader=bgimage&total-slider-slide-group-template='+encodeURIComponent(VPM_SLIDE_GROUP_TEMPLATE)+'&total-slider-slide-group-template-location='+encodeURIComponent(VPM_SLIDE_GROUP_TEMPLATE_LOCATION)+'&type=image&post_id=0&TB_iframe=true&height=400&width=600');return false;});window.send_to_editor=function(html){imgurl=$('img',html).attr('src');var imgTitle=$('img',html).attr('title');var classes=$('img',html).attr('class').split(' ');var attachmentID=parseInt(classes[classes.length-1].substring(9,classes[classes.length-1].length));$('#edit-slide-image-url').val(imgurl);slidePreviewData.background_url=imgurl;slidePreviewData.background_attachment_id=attachmentID;$('#'+editingSlideSortButton).children('.slidesort_slidebox').css('background','url('+imgurl+')');tb_remove();$().updateSlidePreview();}
