...
{alert(result.error);}
else{$().clearForm();$('#edit-slide-title').val(result.title);$('#edit-slide-description').val(result.description);$('#edit-slide-image-url').val(result.background);if(!isNaN(result.link)&&result.link!=0)
...
slidePreviewData.title=result.title;slidePreviewData.description=result.description;slidePreviewData.background_url=result.background;originalBackground=result.background;if(!VPM_SHOULD_DISABLE_XY)
...
else{$.ajax({type:'POST',url:VPM_HPS_PLUGIN_URL+'action=updateSlide&group='+VPM_HPS_GROUP,data:{'id':$('#'+editingSlideSortButton).attr('id').substr($('#'+editingSlideSortButton).attr('id').indexOf('slidesort_')+10,$('#'+editingSlideSortButton).attr('id').length),'title':$('#edit-slide-title').val(),'description':$('#edit-slide-description').val(),'background':$('#edit-slide-image-url').val(),'link':linkToSave,'title_pos_x':calcBoxOffsetLeft,'title_pos_y':calcBoxOffsetTop},success:function(result){if(result.error){alert(result.error);}
