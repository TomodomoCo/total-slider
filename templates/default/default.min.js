;(function($){"use strict";var version='BETA-20120923';$.fn.cycle=function(options){var o;if(this.length===0&&!$.isReady){o={s:this.selector,c:this.context};log('requeuing slideshow (dom not ready)');$(function(){$(o.s,o.c).cycle(options);});return this;}
...
opts=$.extend({},$.fn.cycle.defaults,data,options||{});opts.timeoutId=0;opts.paused=0;opts.container=container;opts._maxZ=opts.maxZ;opts.API=$.extend({_container:container},$.fn.cycle.API);opts.API.log=log;opts.API.trigger=function(eventName,args){opts.container.trigger(eventName,args);return opts.API;};container.data('cycle.opts',opts);container.data('cycle.API',opts.API);opts.API.trigger('cycle-bootstrap',[opts,opts.API]);opts.API.addInitialSlides();opts.API.preInitSlideshow();if(opts.slides.length)
opts.API.initSlideshow();});};$.fn.cycle.API={opts:function(){return this._container.data('cycle.opts');},addInitialSlides:function(){var opts=this.opts();var slides=opts.slides;opts.slideCount=0;opts.slides=$();slides=slides.jquery?slides:opts.container.find(slides);opts.API.add(slides);},preInitSlideshow:function(){var opts=this.opts();opts.API.trigger('cycle-pre-initialize',[opts]);var tx=$.fn.cycle.transitions[opts.fx];if(tx&&$.isFunction(tx.preInit))
...
opts.slides=opts.slides.add(slide);opts.API.initSlide(slideOpts,slide,--opts._maxZ);slide.data('cycle.opts',slideOpts);opts.API.trigger('cycle-slide-added',[opts,slideOpts,slide]);});opts.API.updateView();startSlideshow=opts._preInitialized&&(oldSlideCount<2&&opts.slideCount>=1);if(startSlideshow){if(!opts._initialized)
opts.API.initSlideshow();else if(opts.timeout){opts.timeoutId=setTimeout(function(){var len=opts.slides.length;opts.nextSlide=opts.reverse?len-1:1;opts.API.prepareTx(false,!opts.reverse);},opts.timeout+opts.delay);}}},calcFirstSlide:function(){var opts=this.opts();var firstSlideIndex;firstSlideIndex=parseInt(opts.startingSlide||0,10);if(firstSlideIndex>=opts.slides.length||firstSlideIndex<0)
...
slideOpts.speed=slideOpts.manualSpeed;if(opts.nextSlide!=opts.currSlide&&(manual||(!opts.paused&&opts.timeout))){opts.API.trigger('cycle-before',[slideOpts,curr,next,fwd]);if(tx.before)
tx.before(slideOpts,curr,next,fwd);after=function(){opts.busy=false;if(tx.after)
tx.after(slideOpts,curr,next,fwd);opts.API.trigger('cycle-after',[slideOpts,curr,next,fwd]);opts.API.queueTransition(slideOpts);opts.API.updateView();};opts.busy=true;if(tx.transition)
tx.transition(slideOpts,curr,next,fwd,after);else
opts.API.doTransition(slideOpts,curr,next,fwd,after);opts.API.calcNextSlide();}else{opts.API.queueTransition(slideOpts);}},doTransition:function(slideOpts,currEl,nextEl,fwd,callback){var opts=slideOpts;var curr=$(currEl),next=$(nextEl);var fn=function(){next.animate(opts.animIn||{opacity:1},opts.speed,opts.easeIn||opts.easing,callback);};next.css(opts.cssBefore||{});curr.animate(opts.animOut||{},opts.speed,opts.easeOut||opts.easing,function(){curr.css(opts.cssAfter||{});if(!opts.sync){fn();}});if(opts.sync){fn();}},queueTransition:function(slideOpts){var opts=this.opts();if(opts.nextSlide===0&&--opts.loop===0){opts.API.log('terminating; loop=0');opts.timeout=0;opts.API.trigger('cycle-finished',[opts]);opts.nextSlide=opts.currSlide;return;}
...
slideOpts.speed=slideOpts.speed/2;slide.addClass(opts.slideClass);},updateView:function(){var opts=this.opts();if(!opts._initialized)
...
$(window).off('resize',opts._autoHeightOnResize);});})(jQuery);(function($){"use strict";$.extend($.fn.cycle.defaults,{caption:'> .cycle-caption',captionTemplate:'{{slideNum}} / {{slideCount}}',overlay:'> .cycle-overlay',overlayTemplate:'<div>{{title}}</div><div>{{desc}}</div>'});$(document).on('cycle-update-view',function(e,opts,slideOpts,currSlide){var el;$.each(['caption','overlay'],function(){var name=this,template=slideOpts[name+'Template'];if(opts[name]&&template){el=opts.API.getComponent(name);el.html(opts.API.tmpl(template,slideOpts,currSlide));}});});$(document).on('cycle-destroyed',function(e,opts){var el;$.each(['caption','overlay'],function(){var name=this,template=opts[name+'Template'];if(opts[name]&&template){el=opts.API.getComponent('caption');el.empty();}});});})(jQuery);(function($){"use strict";var c2=$.fn.cycle;$.fn.cycle=function(options){var cmd,cmdFn,opts;var args=$.makeArray(arguments);if($.type(options)=='number'){return this.cycle('goto',options);}
...
return;addFn=opts.API.add;opts.API.add=add;function add(slides,prepend){var slideArr=[];slides=$(slides);var slideCount=slides.length;slides.hide().each(function(){var count=0;var slide=$(this);var images=slide.is('img')?slide:slide.find('img');images=images.filter(':not(.cycle-loader-ignore)');if(!images.length){--slideCount;slideArr.push(slide);return;}
...
opts.nextSlide=nextSlide;opts.API.prepareTx(true,fwd);opts.API.trigger('cycle-pager-activated',[opts,pager,target]);}})(jQuery);(function($){"use strict";$.extend($.fn.cycle.defaults,{next:'> .cycle-next',nextEvent:'click.cycle',disabledClass:'disabled',prev:'> .cycle-prev',prevEvent:'click.cycle',swipe:false});$(document).on('cycle-initialized',function(e,opts){opts.API.getComponent('next').off(opts.nextEvent).on(opts.nextEvent,function(e){e.preventDefault();opts.API.next();});opts.API.getComponent('prev').off(opts.prevEvent).on(opts.prevEvent,function(e){e.preventDefault();opts.API.prev();});if(opts.swipe){opts.container.on('swipeleft.cycle',function(){opts.API.next();});opts.container.on('swiperight.cycle',function(){opts.API.prev();});}});$(document).on('cycle-update-view',function(e,opts,slideOpts,currSlide){if(opts.allowWrap)
...
prev.removeClass(cls).prop('disabled',false);});$(document).on('cycle-destroyed',function(e,opts){$(opts.next).off(opts.nextEvent);$(opts.prev).off(opts.prevEvent);opts.container.off('swipeleft.cycle swiperight.cycle');});})(jQuery);(function($){"use strict";$.extend($.fn.cycle.defaults,{progressive:false});$(document).on('cycle-pre-initialize',function(e,opts){if(!opts.progressive)
...
else{prevFn.apply(opts.API);}};}});})(jQuery);(function($){"use strict";$.extend($.fn.cycle.API,{tmpl:function(str,opts,extra){if(str&&opts){return str.replace(/\{\{((\.)?.*?)\}\}/g,function(_,str){var i,prop,obj=opts,names=str.split('.');if(names.length>1){prop=opts;for(i=0;i<names.length;i++){obj=prop;prop=prop[names[i]]||str;}}else{prop=opts[str];}
if($.isFunction(prop))
return prop.call(obj,opts);if(prop!==undefined&&prop!==null)
return prop;if(extra&&extra[str]!==undefined)
return extra[str];return str;});}}});})(jQuery);jQuery('.ts-default').cycle({fx:'fade',speed:1000,timeout:8000,slides:'.ts-default-slide',});